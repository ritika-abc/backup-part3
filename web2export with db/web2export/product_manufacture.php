...
INNER JOIN supplier ON supplier.username=product.username)
 WHERE  product.mco_id= '$mco_id' AND product.Supply_type='Manufacturer'";
 ...
 ?>   
  ...
  <?php
    ...
    <?php
          include "connection.php";
      $sql = "SELECT product.product_id, product.cat_id, product.sub_cat_id, product.mco_id, product.product, product.username, product.price, product.unit, product.Warranty, product.Diameter, product.Condition, product.Features, product.Supply_type, product.filename, product.date_created, product.search, mco_category.mco_id, mco_category.mco_category, mco_category.filename AS mco FROM product INNER JOIN mco_category ON mco_category.mco_id = product.mco_id WHERE product.Supply_type='Manufacturer'";
  $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        // output data of each row
    while($row = $result->fetch_assoc()) {
 ...
 ?>   
            <a href="product_manufacture.php?data&mco_id=<?php echo $row['mco_id'];?>"><p class="line"><?php echo $row['mco_category'];?>  </p></a>	    
          <?php
                } }
          ?> 
                </table>
