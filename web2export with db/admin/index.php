...
<?php include "header.php"; ?>
  <main id="main" class="main bg-danger-subtle">
   ...
   if(isset($_SESSION["user"])){
      ...
      $sql = "SELECT `limit_id`, COUNT(limit_id) AS buyleads, `buy_id`, `supp_username` FROM `limit_buleads` WHERE 1 ";
 ...
 ?>  
  ...
  <?php
   ...
   if(isset($_SESSION["user"])){
      ...
      $sql = "SELECT `product_id`, `cat_id`, `sub_cat_id`, `mco_id`, `product`, `username`, `price`, `unit`, `Warranty`, `Diameter`, `Condition`, `Features`, `Supply_type`, `filename`, `date_created`, `search`, COUNT(product_id) AS productnum FROM `product` WHERE 1";
  $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        // output data of each row
    while($row = $result->fetch_assoc()) {
